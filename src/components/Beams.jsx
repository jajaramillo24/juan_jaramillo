  ...
  const beamMaterial = useMemo(
    () =>
      extendMaterial(THREE.MeshStandardMaterial, {
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB('#1a0033')),
...
function createStackedPlanesBufferGeometry(n, width, height, spacing, heightSegments) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      positions[vertexOffset * 3 + 0] = xOffset - width / 2;
      positions[vertexOffset * 3 + 1] = y;
      positions[vertexOffset * 3 + 2] = 0;
      uvs[uvOffset * 2 + 0] = uvXOffset;
      uvs[uvOffset * 2 + 1] = v + uvYOffset;
      vertexOffset++;
      uvOffset++;
      ...
      positions[vertexOffset * 3 + 1] = y;
      positions[vertexOffset * 3 + 2] = 0;
      uvs[uvOffset * 2 + 0] = 1 + uvXOffset;
      uvs[uvOffset * 2 + 1] = v + uvYOffset;
      vertexOffset++;
      uvOffset++;

      if (j < heightSegments) {
        const baseIndex = i * (heightSegments + 1) * 2 + j * 2;
